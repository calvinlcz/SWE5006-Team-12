...
type RestOf<T extends ITask | NewTask> = Omit<T, 'dueDate' | 'dateCreated' | 'dateModified'> & {
  dueDate?: string | null;
...
export class TaskService {
  ...
  protected convertDateFromClient<T extends ITask | NewTask | PartialUpdateTask>(task: T): RestOf<T> {
    return {
      ...
      dateCreated: task.dateCreated?.format(DATE_FORMAT) ?? null,
      dateModified: task.dateModified?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTask: RestTask): ITask {
    return {
      ...
      dateCreated: restTask.dateCreated ? dayjs(restTask.dateCreated) : undefined,
      dateModified: restTask.dateModified ? dayjs(restTask.dateModified) : undefined,
    };
  }
