...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'alertTime'> & {
...
export class NotificationService {
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      alertTime: notification.alertTime?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      alertTime: restNotification.alertTime ? dayjs(restNotification.alertTime) : undefined,
    };
  }
